...
export const selectFeatures = async (
  ...
  maxSelectCount?: number /* 最多选择多少个实体 */
) => {
  if (
  ...
  while (true) {
    ...
    // @ts-ignore
    if (maxSelectCount && map._selectFeatures.length >= maxSelectCount) {
      break;
    }
  }
  ...
  // clearHighlight(map);
  // @ts-ignore
  if (maxSelectCount && map._selectFeatures.length > map._selectFeatures) {
    // @ts-ignore
    return map._selectFeatures.slice(0, maxSelectCount)
  } else {
    // @ts-ignore
