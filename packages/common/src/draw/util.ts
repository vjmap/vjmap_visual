...
// 绘制填充椭圆
export const drawEllipseFill = async (
  map: Map,
  draw: IDrawTool,
  options?: Record<string, any>,
  drawProperty?: Record<string, any>,
  ...
  return await drawEllipse(map, draw, options, drawProperty, showInfoFunc, true);
};
// 绘制椭圆
export const drawEllipseEdge = async (
  map: Map,
  draw: IDrawTool,
  options?: Record<string, any>,
  drawProperty?: Record<string, any>,
  ...
  return await drawEllipse(map, draw, options, drawProperty, showInfoFunc, false);
};

// drawEllipseFillArc
export const drawEllipseFillArc = async (
  map: Map,
  draw: IDrawTool,
  options?: Record<string, any>,
  drawProperty?: Record<string, any>,
  ...
  return await drawEllipse(map, draw, options, drawProperty, showInfoFunc, true, true);
};
// 绘制椭圆弧
export const drawEllipseArc = async (
  map: Map,
  draw: IDrawTool,
  options?: Record<string, any>,
  drawProperty?: Record<string, any>,
  ...
  return await drawEllipse(map, draw, options, drawProperty, showInfoFunc, false, true);
};
...
export const interactiveCreateGeom = async (
  ...
  try {
    ...
    if (param.keepGeoSize !== true) {
      ...
      let dblScale = param.drawInitPixelLength ? scaley : Math.min(scalex, scaley);
...
export const createLineTypePolyline = async (
  ...
  if (dbEnt) {
    ...
    }, undefined, param?.renderAccuracy);
...
export const drawText = async (
  ...
  let boundsLength = (drawProperty.height || 50) * 20;
  if (boundsLength < 1000) boundsLength = 1000;
  docBounds = docBounds ?? [-boundsLength, -boundsLength, boundsLength, boundsLength]; // 文档坐标范围，可以根据需要自己定，但要保证是正方形。保证下面绘制的实体在此范围内
